# Product spec v1.3 — plan-level spend approval (name TBD)

> **Status:** draft. Iterates on v1.2 after a verification review: corrects two false claims
> (the sweep is not keeperless for partial remainders; custody on Base is bounded, not absent),
> resolves where keys sign, makes approval channels and identity verification pluggable,
> restores ENS to a load-bearing role, and sets the testnet-first development split.
> Nothing here is agreed until it's promoted to `latest.md`.
> **One line:** Your agent asks for a plan, not a payment.
> **Visualization companion:** [v1-3.html](./v1-3.html) — journeys, screens, architecture diagrams.
