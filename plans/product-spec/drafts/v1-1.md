# Product spec v1.1 — plan-level spend approval (name TBD)

> **Status: superseded by [v1-2.md](./v1-2.md).**
> Draft. Iterates on v1 — same direction, grounded against the live market:
> discovery via the x402 Bazaar, purchases from real mainnet sellers, testnet scoped to the
> envelope and to CI fixtures. Nothing here is agreed until it's promoted to `latest.md`.
> **One line:** Your agent asks for a plan, not a payment.
> **Visualization companion:** [v1-1.html](./v1-1.html) — journeys, screens, architecture diagrams.
